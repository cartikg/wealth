...
function BankCard({ connection, onSync, onDisconnect, source }: {
  connection: any;
  onSync: () => void;
  onDisconnect: () => void;
  ...
  return (
    <View style={[styles.bankCard, isPlaid && { borderColor: 'rgba(0,220,127,0.2)' }]}>
      <View style={styles.bankHeader}>
        <View style={[styles.bankIconWrap, isPlaid && { backgroundColor: 'rgba(0,220,127,0.08)' }]}>
          <Text style={{ fontSize: 26 }}>{em}</Text>
        </View>
        <View style={styles.bankInfo}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            <Text style={styles.bankName}>{connection.bank_name}</Text>
            <View style={[styles.sourceBadge, isPlaid && { backgroundColor: 'rgba(0,220,127,0.12)' }]}>
              <Text style={[styles.sourceBadgeText, isPlaid && { color: '#00dc7f' }]}>
                {isPlaid ? 'Plaid' : 'TrueLayer'}
              </Text>
            </View>
          </View>
          <View style={styles.connectedBadge}>
            <View style={[styles.greenDot, isPlaid && { backgroundColor: '#00dc7f' }]} />
            <Text style={[styles.connectedText, isPlaid && { color: '#00dc7f' }]}>Connected</Text>
          </View>
        </View>
      ...
      <View style={styles.bankFooter}>
        ...
        <TouchableOpacity style={styles.syncBtn} onPress={onSync}>
          <Ionicons name="refresh" size={14} color={isPlaid ? '#00dc7f' : colors.teal} />
          <Text style={[styles.syncBtnText, isPlaid && { color: '#00dc7f' }]}>Sync</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default function BanksScreen() {
  ...
  const loadStatus = useCallback(async () => {
    try {
      const [tlS, plS] = await Promise.all([
        api.getTrueLayerStatus(),
        api.getPlaidStatus(),
      ]);
      setStatus(tlS);
      setPlaidStatus(plS);
  ...
  const handlePlaidConnect = async () => {
    ...
    try {
      const d = await api.createPlaidLinkToken();
      if (d.error) { Alert.alert('Error', d.error); return; }

      const tokenConfig = { token: d.link_token, noLoadingState: false };
      ...
      open({
        onSuccess: async (success: LinkSuccess) => {
          try {
            ...
            const exResult = await api.exchangePlaidToken(
              success.publicToken,
              meta.institution?.id || '',
              meta.institution?.name || 'Plaid Bank',
            );
            if (exResult.ok) {
              setSyncResult({ ok: true, imported: 0, skipped: 0, message: `${exResult.bank_name} connected! Syncing...` });
              await handlePlaidSync();
              await loadStatus();
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
            } else {
              Alert.alert('Error', exResult.error || 'Token exchange failed');
            }
          } catch (e: any) {
            Alert.alert('Connection failed', e.message);
          }
        ...
        onExit: (exit: LinkExit) => {
          if (exit.error) {
            Alert.alert('Plaid Link Error', exit.error.displayMessage || 'Connection cancelled');
          }
        },
      });
    } catch (e: any) {
      Alert.alert('Plaid error', e.message);
    }
  };

  // Provider choice
  const handleConnect = () => {
    const tlConfigured = status?.configured;
    const plConfigured = plaidStatus?.configured;

    if (!tlConfigured && !plConfigured) {
      router.push('/modals/connect-bank');
      return;
    }

    if (tlConfigured && plConfigured) {
      Alert.alert('Choose Provider', 'Which banking provider would you like to use?', [
        { text: 'TrueLayer (UK)', onPress: handleTrueLayerConnect },
        { text: 'Plaid (US/UK/EU)', onPress: handlePlaidConnect },
        { text: 'Cancel', style: 'cancel' },
      ]);
    } else if (tlConfigured) {
  ...
  const handleSyncAll = async () => {
    ...
    try {
      ...
      if ((status?.connections || []).length > 0) promises.push(api.syncTrueLayer());
      if ((plaidStatus?.connections || []).length > 0) promises.push(api.syncPlaid());
      const results = await Promise.all(promises);
      const totalImported = results.reduce((s, r) => s + (r.imported || 0), 0);
      const totalSkipped = results.reduce((s, r) => s + (r.skipped || 0), 0);
      setSyncResult({ ok: true, imported: totalImported, skipped: totalSkipped, accounts: [] });
      await loadStatus();
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (e: any) {
      setSyncResult({ ok: false, error: e.message });
  ...
  // Disconnect
  const handleDisconnect = (id: string, bankName: string, source: 'truelayer' | 'plaid') => {
    Alert.alert(`Disconnect ${bankName}?`, 'Imported transactions will remain.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Disconnect', style: 'destructive', onPress: async () => {
          if (source === 'plaid') {
            await api.disconnectPlaid(id);
          } else {
  ...
  const tlConfigured = status?.configured;
  const plConfigured = plaidStatus?.configured;
  const anyConfigured = tlConfigured || plConfigured;
  const tlConnections = status?.connections || [];
  const plaidConnections = plaidStatus?.connections || [];
  const allConnections = [...tlConnections, ...plaidConnections];
    ...
    >
      ...
      <View style={styles.headerRow}>
        <View style={styles.headerLeft}>
          ...
          {tlConfigured && (
            <View style={[styles.envBadge, { backgroundColor: isSandbox ? colors.primaryDim : colors.tealDim,
              borderColor: isSandbox ? 'rgba(59,130,246,0.3)' : 'rgba(62,207,178,0.3)' }]}>
              <Text style={[styles.envText, { color: isSandbox ? colors.primary : colors.teal }]}>
      ...
      {!anyConfigured && (
        <View style={styles.warningCard}>
          <Text style={styles.warningTitle}>⚠️ No banking providers configured</Text>
          <Text style={styles.warningText}>
            Set up TrueLayer (UK banks) or Plaid (US/UK/EU banks) to connect your bank accounts and auto-import transactions.
          </Text>
          <TouchableOpacity
            style={styles.docsBtn}
            onPress={() => router.push('/modals/connect-bank')}
          >
            <Text style={styles.docsBtnText}>View Setup Instructions →</Text>
          </TouchableOpacity>
        </View>
      )}

      {/* TrueLayer connections */}
      {tlConnections.map((c: any) => (
        <BankCard
          key={c.id}
          connection={c}
          source="truelayer"
          onSync={handleSync}
          onDisconnect={() => handleDisconnect(c.id, c.bank_name, 'truelayer')}
        />
      ))}

      {/* Plaid connections */}
      {plaidConnections.map((c: any) => (
        <BankCard
          key={c.id}
          connection={c}
          source="plaid"
          onSync={handlePlaidSync}
          onDisconnect={() => handleDisconnect(c.id, c.bank_name, 'plaid')}
        />
      ))}

      {/* Empty state */}
      {anyConfigured && allConnections.length === 0 && (
        <View style={styles.emptyCard}>
          ...
          <View style={styles.bankList}>
            {['🟣 Monzo', '🔵 Revolut', '🔴 Barclays', '🔴 HSBC', '🔵 Chase', '🔴 Wells Fargo', '🔴 BofA', '🔵 Citi'].map(b => (
      ...
      <View style={styles.notSupportedCard}>
        ...
        {[
          { name: 'American Express', reason: 'Not in Open Banking' },
...
const styles = StyleSheet.create({
  ...
  warningTitle: { fontSize: 16, fontWeight: '700', color: colors.primary, marginBottom: spacing.sm },
  ...
  docsBtnText: { fontSize: 13, color: colors.primary, fontWeight: '600' },
  ...
  sourceBadgeText: { fontSize: 9, color: colors.teal, fontWeight: '600' },
  ...
  emptyTitle: { fontSize: 22, color: colors.text, marginBottom: spacing.sm },
